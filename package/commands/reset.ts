...
import { intro, password, outro, isCancel, cancel, note, confirm } from "@clack/prompts";
...
export const registerReset = (program: Command) => {
    program
        .command("reset")
        .alias("r")
        .description("Completely wipe the vault — all credentials, metadata and config.")
        .action(async () => {
            ...
            const sure = await confirm({ message: "Are you absolutely sure you want to wipe the vault?" });
            ...
            try {
                ...
                outro(chalk.green("✓ Vault completely wiped. Run 'vaultgen init' to start fresh." ));
            } catch (error: unknown) {
