...
import { intro, select, text, password, spinner, note, outro, isCancel, cancel } from "@clack/prompts";
...
export const registerUpdate = (program: Command) => {
    program
        ...
        .action(async () => {
            ...
            const master = await password({ message: "Enter Master Key to decrypt current data."});
            ...
            const finalUsername = (newUsername as string).trim() || currentData.username;
            const finalPassword = (newPassword as string).trim() || currentData.password;
            try {
